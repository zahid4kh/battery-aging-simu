...
def plot_bus_analysis(csv_file):
    ...
    for bus_id in df['BusID'].unique():
        ...
        fig.suptitle(f'{bus_id} ({route_type}) - Battery Analysis', fontsize = 16, fontweight='bold')
        ...
        # SOC and SOH plot
        ax1 = axes[0, 0]
        ax1.plot(bus_data['Time_Hours'], bus_data['SoC_Percent'], 'b-', linewidth=2, label='SoC (%)')
        ax1_twin = ax1.twinx()
        ax1_twin.plot(bus_data['Time_Hours'], bus_data['SoH'], 'r-', linewidth=2, label='SOH (%)')
        
        #charging indicators
        if len(charging_times) > 0:
            ax1.scatter(charging_times, [95]*len(charging_times), color='green', s=20, label='Charging', zorder=5)
        if len(regen_times) > 0:
            ax1.scatter(regen_times, [90]*len(regen_times), color='blue', s=20, label='Regen', zorder=5)
        ...
        ax2 = axes[0, 1]
        ax2.plot(bus_data['Time_Hours'], bus_data['Temperature'], 'orange', linewidth=2, label='Temperature')
        ax2_twin = ax2.twinx()
        ax2_twin.plot(bus_data['Time_Hours'], bus_data['SoH'], 'purple', linewidth=2, label='SOH')
        ...
        # DOD plots
        ax3 = axes[1, 0]
        ax3.plot(bus_data['Time_Hours'], bus_data['AvgDoD'], 'brown', linewidth=2, label='Avg DoD (%)')
        ax3_twin = ax3.twinx()
        ax3_twin.plot(bus_data['Time_Hours'], bus_data['SoH'], 'red', linewidth=2, label='SOH (%)')
        ...
        ax4 = axes[1, 1]
        ax4.plot(bus_data['Time_Hours'], bus_data['Current'], 'green', linewidth=2, label='Current (A)')
        ax4_twin = ax4.twinx()
        ax4_twin.plot(bus_data['Time_Hours'], bus_data['Voltage'], 'red', linewidth=2, label='Voltage (V)')
        ...
        ax5 = axes[2, 0]
        ax5.plot(bus_data['Time_Hours'], bus_data['CycleCount'], 'purple', linewidth=2, label='Cycle Count')
        ax5_twin = ax5.twinx()
        ax5_twin.plot(bus_data['Time_Hours'], bus_data['AhThroughput'], 'orange', linewidth=2, label='Ah Throughput')
        ...
        ax6 = axes[2, 1]
        ax6.plot(bus_data['Time_Hours'], bus_data['Speed'], 'blue', linewidth=2, label='Speed (km/h)')
        ax6_twin = ax6.twinx()
        ax6_twin.plot(bus_data['Time_Hours'], bus_data['Passengers'], 'red', linewidth=1, label='Passengers')
        ...
        # saving images
        filename = f"{bus_id}_complete_analysis.png"
...
def main():
    # latest: simulation_results_2025-07-28_10-46-34.csv
    csv_file = "simulation_results_2025-07-28_10-46-34.csv"
